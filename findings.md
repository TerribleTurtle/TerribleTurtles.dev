...
*   **Clamping root `html` font-size & setting `display: flex` on `html`:** Rejected. Setting `font-size: clamp(...)` with `vw` on `html` causes "double-scaling" where all `rem` values inside child `clamp()` functions scale exponentially, destroying 1080p+ layouts. Always define fluid type on `body` or specific selectors, leaving `html` at 100%. Additionally, applying `display: flex; flex-direction: column` simultaneously to both `html` and `body` traps `body` as a flex-child, breaking sticky headers and pointer events.
